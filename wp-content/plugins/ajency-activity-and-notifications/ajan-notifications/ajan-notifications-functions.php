 ...
 *         on failure.
 */
function ajan_notifications_add_notification( $args = array() ) {
	...
	if ( ! $r['allow_duplicate'] ) {
		...
		// duplicate status
		$existing = AJAN_Notifications_Notification::get( array(
			'user_id'           => $r['user_id'],
			'item_id'           => $r['item_id'],
	...
	$notification                    = new AJAN_Notifications_Notification;
 ...
 * @return AJAN_Notifications_Notification
 */
function ajan_notifications_get_notification( $id ) {
	return new AJAN_Notifications_Notification( $id );
}
 ...
 */
function ajan_notifications_delete_notification( $id ) {
	if ( ! ajan_notifications_check_notification_access( ajan_loggedin_user_id(), $id ) ) {
		return false;
	}

	return AJAN_Notifications_Notification::delete( array( 'id' => $id ) );
}
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_mark_notification( $id, $is_new = false ) {
	if ( ! ajan_notifications_check_notification_access( ajan_loggedin_user_id(), $id ) ) {
		return false;
	}

	return AJAN_Notifications_Notification::update(
		array( 'is_new' => $is_new ),
		array( 'id'     => $id     )
	);
}
 ...
 */
function ajan_notifications_get_notifications_for_user( $user_id, $format = 'string' ) {

	// Setup local variables
	$ajan = activitynotifications();

	// Get notifications out of the cache, or query if necessary
	$notifications = wp_cache_get( 'all_for_user_' . $user_id, 'ajan_notifications' );
	if ( false === $notifications ) {
		$notifications = AJAN_Notifications_Notification::get( array(
			'user_id' => $user_id
		) );
		wp_cache_set( 'all_for_user_' . $user_id, $notifications, 'ajan_notifications' );
	}
	...
	foreach ( $grouped_notifications as $component_name => $action_arrays ) {
		...
		foreach ( (array) $action_arrays as $component_action_name => $component_action_items ) {
			...
			//edited to implement registered components for notifications and to provide a way for theme to add callback to custom components
			 //if ( isset( $ajan->{$component_name}->notification_callback ) && is_callable( $ajan->{$component_name}->notification_callback ) ) {
			 if ( isset( $ajan->{'notifications'}->notification_components[$component_name] ) && is_callable( $ajan->{'notifications'}->notification_components[$component_name] ) ) {
				...
				if ( 'object' === $format ) {
					...
					$content = call_user_func(
						$ajan->{'notifications'}->notification_components[$component_name],
					...
					if ( is_string( $content ) ) {
						...
						$notification_object->href    = ajan_loggedin_user_domain();
					} else {
			...
			} elseif ( isset( $ajan->{$component_name}->format_notification_function ) && function_exists( $ajan->{$component_name}->format_notification_function ) ) {
				$renderable[] = call_user_func( $ajan->{$component_name}->format_notification_function, $component_action_name, $component_action_items[0]->item_id, $component_action_items[0]->secondary_item_id, $action_item_count );
			...
			// Allow non BuddyPress components to hook in
				...
				if ( 'object' === $format ) {
					...
					// Retrieve the content of the notification using the callback
					$content = apply_filters_ref_array( 'ajan_notifications_get_notifications_for_user', $ref_array );
				...
				// Return an array of content strings
					...
					$renderable[] = apply_filters_ref_array( 'ajan_notifications_get_notifications_for_user', $ref_array );
				}
			}
		}
	}
	...
	// Filter and return
	return apply_filters( 'ajan_core_get_notifications_for_user', $renderable, $user_id, $format );
}
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_delete_notifications_by_type( $user_id, $component_name, $component_action ) {
	return AJAN_Notifications_Notification::delete( array(
		'user_id'          => $user_id,
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_delete_notifications_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id = false ) {
	return AJAN_Notifications_Notification::delete( array(
		'user_id'           => $user_id,
		'item_id'           => $item_id,
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_delete_all_notifications_by_type( $item_id, $component_name, $component_action = false, $secondary_item_id = false ) {
	return AJAN_Notifications_Notification::delete( array(
		'item_id'           => $item_id,
		'secondary_item_id' => $secondary_item_id,
		'component_name'    => $component_name,
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_delete_notifications_from_user( $user_id, $component_name, $component_action ) {
	return AJAN_Notifications_Notification::delete( array(
		'item_id'           => $user_id,
		'component_name'    => $component_name,
		'component_action'  => $component_action,
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_mark_notifications_by_type( $user_id, $component_name, $component_action, $is_new = false ) {
	return AJAN_Notifications_Notification::update(
		array(
			'is_new' => $is_new
		),
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_mark_notifications_by_item_id( $user_id, $item_id, $component_name, $component_action, $secondary_item_id = false, $is_new = false ) {
	return AJAN_Notifications_Notification::update(
		array(
			'is_new' => $is_new
		),
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_mark_all_notifications_by_type( $item_id, $component_name, $component_action = false, $secondary_item_id = false, $is_new = false ) {
	return AJAN_Notifications_Notification::update(
		array(
			'is_new' => $is_new
		),
 ...
 * @return bool True on success, false on failure.
 */
function ajan_notifications_mark_notifications_from_user( $user_id, $component_name, $component_action, $is_new = false ) {
	return AJAN_Notifications_Notification::update(
		array(
			'is_new' => $is_new
		),
 ...
 */
function ajan_notifications_check_notification_access( $user_id, $notification_id ) {
	return (bool) AJAN_Notifications_Notification::check_access( $user_id, $notification_id );
}
 ...
 */
function ajan_notifications_get_unread_notification_count( $user_id = 0 ) {
	...
	return apply_filters( 'ajan_notifications_get_total_notification_count', $count );
}
 ...
 *
 * @since ajency-activity-and-notifications (1.9.1)
 *
 * @see http://buddypress.trac.wordpress.org/ticket/5300
 */
function ajan_notifications_get_registered_components() {
	...
	$active_components = array_keys( $ajan->active_components );

	// Loop through components, look for callbacks, add to return value
	foreach ( $active_components as $component ) {
		if ( !empty( $ajan->$component->notification_callback ) ) {
			$component_names[] = $component;
		}
	}

	// Return active components with registered notifications callbacks
	return apply_filters( 'ajan_notifications_get_registered_components', $component_names, $active_components );
}
