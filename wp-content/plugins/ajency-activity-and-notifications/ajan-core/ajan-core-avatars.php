 ...
 */
function ajan_core_set_avatar_constants() {

	$ajan = activitynotifications();

	if ( !defined( 'AJAN_AVATAR_THUMB_WIDTH' ) )
		define( 'AJAN_AVATAR_THUMB_WIDTH', 50 );

	if ( !defined( 'AJAN_AVATAR_THUMB_HEIGHT' ) )
		define( 'AJAN_AVATAR_THUMB_HEIGHT', 50 );

	if ( !defined( 'AJAN_AVATAR_FULL_WIDTH' ) )
		define( 'AJAN_AVATAR_FULL_WIDTH', 150 );

	if ( !defined( 'AJAN_AVATAR_FULL_HEIGHT' ) )
		define( 'AJAN_AVATAR_FULL_HEIGHT', 150 );

	if ( !defined( 'AJAN_AVATAR_ORIGINAL_MAX_WIDTH' ) )
		define( 'AJAN_AVATAR_ORIGINAL_MAX_WIDTH', 450 );

	if ( !defined( 'AJAN_AVATAR_ORIGINAL_MAX_FILESIZE' ) ) {

		if ( !isset( $ajan->site_options['fileupload_maxk'] ) ) {
			define( 'AJAN_AVATAR_ORIGINAL_MAX_FILESIZE', 5120000 ); // 5mb
		} else {
			define( 'AJAN_AVATAR_ORIGINAL_MAX_FILESIZE', $ajan->site_options['fileupload_maxk'] * 1024 );
		}
	}

	if ( ! defined( 'AJAN_SHOW_AVATARS' ) ) {
		define( 'AJAN_SHOW_AVATARS', ajan_get_option( 'show_avatars' ) );
	}
}
add_action( 'ajan_init', 'ajan_core_set_avatar_constants', 3 );
 ...
 */
function ajan_core_set_avatar_globals() {
	$ajan = activitynotifications();

	$ajan->avatar        = new stdClass;
	$ajan->avatar->thumb = new stdClass;
	$ajan->avatar->full  = new stdClass;

	// Dimensions
	$ajan->avatar->thumb->width  = AJAN_AVATAR_THUMB_WIDTH;
	$ajan->avatar->thumb->height = AJAN_AVATAR_THUMB_HEIGHT;
	$ajan->avatar->full->width   = AJAN_AVATAR_FULL_WIDTH;
	$ajan->avatar->full->height  = AJAN_AVATAR_FULL_HEIGHT;

	// Upload maximums
	$ajan->avatar->original_max_width    = AJAN_AVATAR_ORIGINAL_MAX_WIDTH;
	$ajan->avatar->original_max_filesize = AJAN_AVATAR_ORIGINAL_MAX_FILESIZE;

	// Defaults
	$ajan->avatar->thumb->default = ajan_core_avatar_default_thumb();
	$ajan->avatar->full->default  = ajan_core_avatar_default();

	// These have to be set on page load in order to avoid infinite filter loops at runtime
	$ajan->avatar->upload_path = ajan_core_avatar_upload_path();
	$ajan->avatar->url = ajan_core_avatar_url();

	// Cache the root blog's show_avatars setting, to avoid unnecessary
	// calls to switch_to_blog()
	$ajan->avatar->show_avatars = (bool) AJAN_SHOW_AVATARS;

	// Backpat for pre-1.5
	if ( ! defined( 'AJAN_AVATAR_UPLOAD_PATH' ) )
		define( 'AJAN_AVATAR_UPLOAD_PATH', $ajan->avatar->upload_path );

	// Backpat for pre-1.5
	if ( ! defined( 'AJAN_AVATAR_URL' ) )
		define( 'AJAN_AVATAR_URL', $ajan->avatar->url );

	do_action( 'ajan_core_set_avatar_globals' );
}
add_action( 'ajan_setup_globals', 'ajan_core_set_avatar_globals' );

/**
 * Get an avatar for a ActivityNotifications object.
 *
 ...
 * locally:
 *    add_filter( 'ajan_core_fetch_avatar_no_grav', '__return_true' );
 *
 * @param array $args {
 *     An array of arguments. All arguments are technically optional; some
 *     will, if not provided, be auto-detected by ajan_core_fetch_avatar(). This
 ...
 *           avatar. ActivityNotifications natively supports three options: 'user',
 ...
 *           {@link ajan_get_activity_avatar()} for an example. Default: ''.
 ...
 *           "{$item_id}-{$object}@{ajan_get_root_domain()}". The user query adds
 ...
 *           fallbacks globally, see the 'ajan_core_fetch_avatar_no_grav' filter.
 *           Default: false.
 ...
 */
function ajan_core_fetch_avatar( $args = '' ) {

	// If avatars are disabled for the root site, obey that request and bail
	if ( ! activitynotifications()->avatar->show_avatars )
		return;
	...
	if ( empty( $item_id ) ) {
		...
		switch ( $object ) {
			...
			case 'group' :
				if ( ajan_is_active( 'groups' ) ) {
					$item_id = $ajan->groups->current_group->id;
				} else {
					$item_id = false;
				}
		...
		$item_id = apply_filters( 'ajan_core_avatar_item_id', $item_id, $object, $params );
	...
	$class = apply_filters( 'ajan_core_avatar_class', $class, $item_id, $object, $params );
	...
	if ( empty( $avatar_dir ) ) {
		...
		$avatar_dir = apply_filters( 'ajan_core_avatar_dir', $avatar_dir, $object, $params );
	...
	if ( false !== strpos( $alt, '%s' ) || false !== strpos( $alt, '%1$s' ) ) {
		...
		switch ( $object ) {
			...
			default : 
				...
				$item_name = $user_info->display_name;
				break;
		}

		$item_name = apply_filters( 'ajan_core_avatar_alt', $item_name, $item_id, $object, $params );
	...
	if ( empty( $alt ) )
		$alt = __( 'Avatar Image', 'ajency-activity-and-notifications' );

	$html_alt = ' alt="' . esc_attr( $alt ) . '"';

	// Set title tag, if it's been provided
	if ( !empty( $title ) ) {
		$title = " title='" . esc_attr( apply_filters( 'ajan_core_avatar_title', $title, $item_id, $object, $params ) ) . "'";
	}
	...
	if ( false !== $width ) {
		...
		$html_width = ' width="' . ajan_core_avatar_thumb_width() . '"';
	} else {
		$html_width = ' width="' . ajan_core_avatar_full_width() . '"';
	}
	...
	if ( false !== $height ) {
		...
		$html_height = ' height="' . ajan_core_avatar_thumb_height() . '"';
	} else {
		$html_height = ' height="' . ajan_core_avatar_full_height() . '"';
	}

	// Set img URL and DIR based on prepopulated constants
	$avatar_loc        = new stdClass();
	$avatar_loc->path  = trailingslashit( ajan_core_avatar_upload_path() );
	$avatar_loc->url   = trailingslashit( ajan_core_avatar_url() );

	$avatar_loc->dir   = trailingslashit( $avatar_dir );
	$avatar_folder_url = apply_filters( 'ajan_core_avatar_folder_url', ( $avatar_loc->url  . $avatar_loc->dir . $item_id ), $item_id, $object, $avatar_dir );
	$avatar_folder_dir = apply_filters( 'ajan_core_avatar_folder_dir', ( $avatar_loc->path . $avatar_loc->dir . $item_id ), $item_id, $object, $avatar_dir );
	...
	if ( file_exists( $avatar_folder_dir ) ) {
		...
		if ( isset( $avatar_url ) ) {
			...
			if ( true === $html ) {
				return apply_filters( 'ajan_core_fetch_avatar', '<img src="' . $avatar_url . '" class="' . esc_attr( $class ) . '"' . $css_id . $html_width . $html_height . $html_alt . $title . ' />', $params, $item_id, $avatar_dir, $css_id, $html_width, $html_height, $avatar_folder_url, $avatar_folder_dir );

			// ...or only the URL
			} else {
				return apply_filters( 'ajan_core_fetch_avatar_url', $avatar_url, $params );
			}
		}
	}
	...
	if ( ! apply_filters( 'ajan_core_fetch_avatar_no_grav', $no_grav ) ) {

		// Set gravatar size
		if ( false !== $width ) {
			$grav_size = $width;
		} else if ( 'full' == $type ) {
			$grav_size = ajan_core_avatar_full_width();
		} else if ( 'thumb' == $type ) {
			$grav_size = ajan_core_avatar_thumb_width();
		}

		// Set gravatar type
		if ( empty( $ajan->grav_default->{$object} ) ) {
			$default_grav = 'wavatar';
		} else if ( 'mystery' == $ajan->grav_default->{$object} ) {
			$default_grav = apply_filters( 'ajan_core_mysteryman_src', 'mm', $grav_size );
		} else {
			$default_grav = $ajan->grav_default->{$object};
		}
		...
		if ( empty( $email ) ) {
			if ( 'user' == $object ) { 
				...
				$email = "{$item_id}-{$object}@{ajan_get_root_domain()}";
			}
		}
		...
		$email    = apply_filters( 'ajan_core_gravatar_email', $email, $item_id, $object );
		$gravatar = apply_filters( 'ajan_gravatar_url', $host ) . md5( strtolower( $email ) ) . '?d=' . $default_grav . '&amp;s=' . $grav_size;
	...
	// No avatar was found, and we've been told not to use a gravatar.
		...
		$gravatar = apply_filters( "ajan_core_default_avatar_$object", ajan_core_avatar_default( 'local' ), $params );
	}

	if ( true === $html ) {
		return apply_filters( 'ajan_core_fetch_avatar', '<img src="' . $gravatar . '" class="' . esc_attr( $class ) . '"' . $css_id . $html_width . $html_height . $html_alt . $title . ' />', $params, $item_id, $avatar_dir, $css_id, $html_width, $html_height, $avatar_folder_url, $avatar_folder_dir );
	} else {
		return apply_filters( 'ajan_core_fetch_avatar_url', $gravatar, $params );
	}
}
 ...
 */
function ajan_core_delete_existing_avatar( $args = '' ) {
	...
	if ( empty( $item_id ) ) {
		if ( 'user' == $object )
			...
			$item_id = activitynotifications()->groups->current_group->id;
		else if ( 'blog' == $object )
			$item_id = $current_blog->id;

		$item_id = apply_filters( 'ajan_core_avatar_item_id', $item_id, $object );
	...
	if ( empty( $avatar_dir ) ) {
		...
		$avatar_dir = apply_filters( 'ajan_core_avatar_dir', $avatar_dir, $object );

		if ( !$avatar_dir ) return false;
	}

	$avatar_folder_dir = apply_filters( 'ajan_core_avatar_folder_dir', ajan_core_avatar_upload_path() . '/' . $avatar_dir . '/' . $item_id, $item_id, $object, $avatar_dir );
	...
	do_action( 'ajan_core_delete_existing_avatar', $args );
 ...
 * properly using ajan_core_check_avatar_upload(). It then checks that the file
 ...
 * location.
 *
 * @see ajan_core_check_avatar_upload()
 * @see ajan_core_check_avatar_type()
 *
 * @param array $file The appropriate entry the from $_FILES superglobal.
 * @param string $upload_dir_filter A filter to be applied to 'upload_dir'.
 * @return bool True on success, false on failure.
 */
function ajan_core_avatar_handle_upload( $file, $upload_dir_filter ) {

	/***
	 * You may want to hook into this filter if you want to override this function.
	 * Make sure you return false.
	 */
	if ( !apply_filters( 'ajan_core_pre_avatar_handle_upload', true, $file, $upload_dir_filter ) )
		return true;

	require_once( ABSPATH . '/wp-admin/includes/file.php' );

	$uploadErrors = array(
		0 => __( 'The image was uploaded successfully', 'ajency-activity-and-notifications' ),
		1 => __( 'The image exceeds the maximum allowed file size of: ', 'ajency-activity-and-notifications' ) . size_format( ajan_core_avatar_original_max_filesize() ),
		2 => __( 'The image exceeds the maximum allowed file size of: ', 'ajency-activity-and-notifications' ) . size_format( ajan_core_avatar_original_max_filesize() ),
		3 => __( 'The uploaded file was only partially uploaded.', 'ajency-activity-and-notifications' ),
		4 => __( 'The image was not uploaded.', 'ajency-activity-and-notifications' ),
		6 => __( 'Missing a temporary folder.', 'ajency-activity-and-notifications' )
	);

	if ( ! ajan_core_check_avatar_upload( $file ) ) {
		ajan_core_add_message( sprintf( __( 'Your upload failed, please try again. Error was: %s', 'ajency-activity-and-notifications' ), $uploadErrors[$file['file']['error']] ), 'error' );
		return false;
	}

	if ( ! ajan_core_check_avatar_size( $file ) ) {
		ajan_core_add_message( sprintf( __( 'The file you uploaded is too big. Please upload a file under %s', 'ajency-activity-and-notifications' ), size_format( ajan_core_avatar_original_max_filesize() ) ), 'error' );
		return false;
	}

	if ( ! ajan_core_check_avatar_type( $file ) ) {
		ajan_core_add_message( __( 'Please upload only JPG, GIF or PNG photos.', 'ajency-activity-and-notifications' ), 'error' );
		return false;
	}
	...
	$ajan->avatar_admin->original = wp_handle_upload( $file['file'], array( 'action'=> 'ajan_avatar_upload' ) );
	...
	if ( !empty( $ajan->avatar_admin->original['error'] ) ) {
		ajan_core_add_message( sprintf( __( 'Upload Failed! Error was: %s', 'ajency-activity-and-notifications' ), $ajan->avatar_admin->original['error'] ), 'error' );
		return false;
	}

	// Get image size
	$size  = @getimagesize( $ajan->avatar_admin->original['file'] );
	$error = false;

	// Check image size and shrink if too large
	if ( $size[0] > ajan_core_avatar_original_max_width() ) {
		$editor = wp_get_image_editor( $ajan->avatar_admin->original['file'] );

		if ( ! is_wp_error( $editor ) ) {
			$editor->set_quality( 100 );

			$resized = $editor->resize( ajan_core_avatar_original_max_width(), ajan_core_avatar_original_max_width(), false );
			...
			if ( false === $error ) {
				$ajan->avatar_admin->resized = $thumb;
			}
		...
		if ( false !== $error ) {
			ajan_core_add_message( sprintf( __( 'Upload Failed! Error was: %s', 'ajency-activity-and-notifications' ), $error->get_error_message() ), 'error' );
			return false;
		}
	}

	if ( ! isset( $ajan->avatar_admin->image ) )
		$ajan->avatar_admin->image = new stdClass();

	// We only want to handle one image after resize.
	if ( empty( $ajan->avatar_admin->resized ) ) {
		$ajan->avatar_admin->image->dir = str_replace( ajan_core_avatar_upload_path(), '', $ajan->avatar_admin->original['file'] );
	} else {
		$ajan->avatar_admin->image->dir = str_replace( ajan_core_avatar_upload_path(), '', $ajan->avatar_admin->resized['path'] );
		@unlink( $ajan->avatar_admin->original['file'] );
	}

	// Check for WP_Error on what should be an image
	if ( is_wp_error( $ajan->avatar_admin->image->dir ) ) {
		ajan_core_add_message( sprintf( __( 'Upload failed! Error was: %s', 'ajency-activity-and-notifications' ), $ajan->avatar_admin->image->dir->get_error_message() ), 'error' );
		return false;
	}

	// Set the url value for the image
	$ajan->avatar_admin->image->url = ajan_core_avatar_url() . $ajan->avatar_admin->image->dir;
 ...
 *           as retrieved by ajan_core_avatar_full_width().
 *     @type int $crop_h Crop height. Default: the global 'full' avatar height,
 *           as retrieved by ajan_core_avatar_full_height().
 ...
 * @return bool True on success, false on failure.
 */
function ajan_core_avatar_handle_crop( $args = '' ) {
	...
	$r = wp_parse_args( $args, array(
		...
		'item_id'       => false,
		'original_file' => false,
		'crop_w'        => ajan_core_avatar_full_width(),
		'crop_h'        => ajan_core_avatar_full_height(),
		'crop_x'        => 0,
		'crop_y'        => 0
	) );
	 ...
	 * Make sure you return false.
	 */
	if ( !apply_filters( 'ajan_core_pre_avatar_handle_crop', true, $r ) )
		return true;
	...
	$original_file = ajan_core_avatar_upload_path() . $original_file;

	if ( !file_exists( $original_file ) )
		return false;

	if ( empty( $item_id ) ) {
		$avatar_folder_dir = apply_filters( 'ajan_core_avatar_folder_dir', dirname( $original_file ), $item_id, $object, $avatar_dir );
	} else {
		$avatar_folder_dir = apply_filters( 'ajan_core_avatar_folder_dir', ajan_core_avatar_upload_path() . '/' . $avatar_dir . '/' . $item_id, $item_id, $object, $avatar_dir );
	}
	...
	$existing_avatar = ajan_core_fetch_avatar( array(
		'object'  => $object,
		'item_id' => $item_id,
		'html' => false,
	) );
	...
	if ( ! empty( $existing_avatar ) ) {
		...
		if ( $existing_avatar_path !== $new_avatar_path ) {
			ajan_core_delete_existing_avatar( array( 'object' => $object, 'item_id' => $item_id, 'avatar_path' => $avatar_folder_dir ) );
		}
	}
	...
	// Crop the image
	$full_cropped  = wp_crop_image( $original_file, (int) $crop_x, (int) $crop_y, (int) $crop_w, (int) $crop_h, ajan_core_avatar_full_width(),  ajan_core_avatar_full_height(),  false, $avatar_folder_dir . '/' . $full_filename  );
	$thumb_cropped = wp_crop_image( $original_file, (int) $crop_x, (int) $crop_y, (int) $crop_w, (int) $crop_h, ajan_core_avatar_thumb_width(), ajan_core_avatar_thumb_height(), false, $avatar_folder_dir . '/' . $thumb_filename );
 ...
 */
function ajan_core_fetch_avatar_filter( $avatar, $user, $size, $default, $alt = '' ) {
	global $pagenow;
	...
	if ( empty( $alt ) ) { 
				...
				$item_name = $user_info->display_name;
		$alt = sprintf( __( 'Avatar of %s', 'ajency-activity-and-notifications' ), $item_name );
	}

	// Use the 'thumb' type, unless the requested width is bigger than
	// BP's thumb width.
	$type = 'thumb';
	if ( (int) $size > ajan_core_avatar_thumb_width() ) {
		$type = 'full';
	}

	// Let ActivityNotifications handle the fetching of the avatar
	$ajan_avatar = ajan_core_fetch_avatar( array(
		'item_id' => $id,
		'type'    => $type,
		'width'   => $size,
		'height'  => $size,
	...
	// If ActivityNotifications found an avatar, use it. If not, use the result of get_avatar
	return ( !$ajan_avatar ) ? $avatar : $ajan_avatar;
}
add_filter( 'get_avatar', 'ajan_core_fetch_avatar_filter', 10, 5 );
 ...
 */
function ajan_core_check_avatar_upload( $file ) {
 ...
 */
function ajan_core_check_avatar_size( $file ) {
	if ( $file['file']['size'] > ajan_core_avatar_original_max_filesize() )
		return false;
 ...
 */
function ajan_core_check_avatar_type($file) {
 ...
 * @uses wp_upload_dir()
 *
 * @param string $type The variable we want to return from the $ajan->avatars
 *        object. Only 'upload_path' and 'url' are supported. Default: 'upload_path'.
 * @return string The avatar upload directory path.
 */
function ajan_core_get_upload_dir( $type = 'upload_path' ) {
	$ajan = activitynotifications();

	switch ( $type ) {
		case 'upload_path' :
			$constant = 'AJAN_AVATAR_UPLOAD_PATH';
			$key      = 'basedir';

			break;

		case 'url' :
			$constant = 'AJAN_AVATAR_URL';
			$key      = 'baseurl';
	...
	// See if the value has already been calculated and stashed in the $ajan global
	if ( isset( $ajan->avatar->$type ) ) {
		$retval = $ajan->avatar->$type;
	} else {
		...
		if ( defined( $constant ) ) {
			...
			if ( ! empty( $ajan->avatar->upload_dir ) ) {
				$upload_dir = $ajan->avatar->upload_dir;
			...
			// No cache, so query for it
				...
				$ajan->avatar->upload_dir = $upload_dir;
			}
		...
		// Stash in $ajan for later use
		$ajan->avatar->$type = $retval;
	}
 ...
 */
function ajan_core_avatar_upload_path() {
	return apply_filters( 'ajan_core_avatar_upload_path', ajan_core_get_upload_dir() );
}
 ...
 */
function ajan_core_avatar_url() {
	return apply_filters( 'ajan_core_avatar_url', ajan_core_get_upload_dir( 'url' ) );
}
 ...
 */
function ajan_get_user_has_avatar( $user_id = 0 ) {

	if ( empty( $user_id ) )
		$user_id = ajan_displayed_user_id();

	$retval = false;
	if ( ajan_core_fetch_avatar( array( 'item_id' => $user_id, 'no_grav' => true, 'html' => false ) ) != ajan_core_avatar_default( 'local' ) )
		$retval = true;

	return (bool) apply_filters( 'ajan_get_user_has_avatar', $retval, $user_id );
}
 ...
 */
function ajan_core_avatar_dimension( $type = 'thumb', $h_or_w = 'height' ) {
	$ajan  = activitynotifications();
	$dim = isset( $ajan->avatar->{$type}->{$h_or_w} ) ? (int) $ajan->avatar->{$type}->{$h_or_w} : false;

	return apply_filters( 'ajan_core_avatar_dimension', $dim, $type, $h_or_w );
}
 ...
 */
function ajan_core_avatar_thumb_width() {
	return apply_filters( 'ajan_core_avatar_thumb_width', ajan_core_avatar_dimension( 'thumb', 'width' ) );
}
 ...
 */
function ajan_core_avatar_thumb_height() {
	return apply_filters( 'ajan_core_avatar_thumb_height', ajan_core_avatar_dimension( 'thumb', 'height' ) );
}
 ...
 */
function ajan_core_avatar_full_width() {
	return apply_filters( 'ajan_core_avatar_full_width', ajan_core_avatar_dimension( 'full', 'width' ) );
}
 ...
 */
function ajan_core_avatar_full_height() {
	return apply_filters( 'ajan_core_avatar_full_height', ajan_core_avatar_dimension( 'full', 'height' ) );
}
 ...
 */
function ajan_core_avatar_original_max_width() {
	return apply_filters( 'ajan_core_avatar_original_max_width', (int) activitynotifications()->avatar->original_max_width );
}
 ...
 */
function ajan_core_avatar_original_max_filesize() {
	return apply_filters( 'ajan_core_avatar_original_max_filesize', (int) activitynotifications()->avatar->original_max_filesize );
}
 ...
 */
function ajan_core_avatar_default( $type = 'gravatar' ) {
	// Local override
	if ( defined( 'AJAN_AVATAR_DEFAULT' ) ) {
		$avatar = AJAN_AVATAR_DEFAULT;

	// Use the local default image
	} else if ( 'local' === $type ) {
		$avatar = activitynotifications()->plugin_url . 'ajan-core/images/mystery-man.jpg';
	...
	// Use Gravatar's mystery man as fallback
		...
		$avatar = $host . '/avatar/00000000000000000000000000000000?d=mm&amp;s=' . ajan_core_avatar_full_width();
	}

	return apply_filters( 'ajan_core_avatar_default', $avatar );
}

/**
 * Get the URL of the 'thumb' default avatar.
 *
 * Uses Gravatar's mystery-man avatar, unless AJAN_AVATAR_DEFAULT_THUMB has been
 * defined.
 *
 * @since ActivityNotifications (1.5.0)
 *
 ...
 */
function ajan_core_avatar_default_thumb( $type = 'gravatar' ) {
	// Local override
	if ( defined( 'AJAN_AVATAR_DEFAULT_THUMB' ) ) {
		$avatar = AJAN_AVATAR_DEFAULT_THUMB;

	// Use the local default image
	} else if ( 'local' === $type ) {
		$avatar = activitynotifications()->plugin_url . 'ajan-core/images/mystery-man-50.jpg';
	...
	// Use Gravatar's mystery man as fallback
		...
		$avatar = $host . '/avatar/00000000000000000000000000000000?d=mm&amp;s=' . ajan_core_avatar_thumb_width();
	}

	return apply_filters( 'ajan_core_avatar_thumb', $avatar );
}
