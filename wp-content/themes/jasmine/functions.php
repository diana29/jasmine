...
$args = array(
	...
	ajan_notifications_add_notification($args); 
}


//add_action('init','test_notification');
function test_activity(){
global $user_ID;
$args = array(
		 
		'action'            => '@jasmine updated the funds.........',    // The activity action - e.g. "Jon Doe posted an update"
		'content'           => '@jasmine',    // Optional: The content of the activity item e.g. "BuddyPress is awesome guys!"

		'component'         => 'funds', // The name/ID of the component e.g. groups, profile, mycomponent
		'type'              => 'funds_update', // The activity type e.g. activity_update, profile_updated
...
// add_action('init','test_activity');


function  activity_screen_my_activity(){
 global $user_ID;
 
 //formatted_echo(ajan_get_activity_by_id(26));
$args = array( 
		'content'     => 'fdgfdgfg', // the comment by the user 
		'activity_id' => 1, // ID of the root activity item
		'parent_id'   => false   // ID of a parent comment (optional)
	) ;
 
//ajan_activity_new_comment($args);
}
 add_action('init','activity_screen_my_activity');

function register_theme_notification_actions($args){

	$args[] = array(	'component_id'		=>	'friends', 
						'format_callback'	=>	'notification_format_notification_action_friends'
					);
	$args[] = array(	'component_id'		=>	'funds', 
						'format_callback'	=>	'notification_format_notification_action_funds'
					); 
	return $args;

}
add_filter('ajan_register_theme_notification_actions','register_theme_notification_actions',11,1);

function activity_format_activity_action_funds_update( $action, $activity){
 ...
 * @return array|string
 */
function notification_format_notification_action_friends( $action, $item_id, $secondary_item_id, $total_items, $format = 'string' ) {
	...
	switch ( $action ) {
		...
		case 'friendship_request':
			...
			if ( (int) $total_items > 1 ) {
				...
				$text = sprintf( __( 'You have a friendship request from %s', 'buddypress' ),   ( $item_id ) );
...
//register theme activity components

function register_theme_activity_actions($args){

	$args = array(
				array(	'component_id'		=>	'post',
						'type'				=>	'post_published',
						'description'		=>	'Blog post published',
						'format_callback'	=>	'activity_format_activity_action_post_published'
					),
			); 
	return $args;
...
//record theme activity post_published

function record_blog_post_published_activity( $ID, $post ) {
  	...
  	$args = array(
						...
						'action'            => $creator_user_info->display_name.' published blog post <a href="'. get_permalink( $ID).'">'.$post->post_title.'</a>',    // The activity action - e.g. "Jon Doe posted an update"
						
						'component'         => 'post', // The name/ID of the component e.g. groups, profile, mycomponent
						
						'type'              => 'post_published', // The activity type e.g. activity_update, profile_updated
...
add_action(  'publish_post',  'record_blog_post_published_activity', 10, 2 );
